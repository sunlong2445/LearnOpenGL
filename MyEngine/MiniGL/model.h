...
public:
    ...
    void OnCenter(glm::vec3 CameraPos, glm::vec3 direction,float fov,float AspectRatio)
    {
       //竖直方向dis
        float aabb_height = aabb.max.y - aabb.min.y;
        float distance_y = 4*aabb_height / glm::tan(glm::radians(fov));
        //水平方向的dis
        float aabb_width =aabb.max.x - aabb.min.x;
        float distance_x = 4 * aabb_width / (glm::tan(glm::radians(fov))* AspectRatio);
    ...
    void GetAABB()
    {
        ...
        for (int i = 0; i < meshes.size(); i++)
        {
            ...
            {
                // 计算pMax
                if (xmax < meshes[i].vertices[j].Position.x)
                    xmax = meshes[i].vertices[j].Position.x;
                if (ymax < meshes[i].vertices[j].Position.y)
                    ymax = meshes[i].vertices[j].Position.y;
                if (zmax < meshes[i].vertices[j].Position.z)
                    zmax = meshes[i].vertices[j].Position.z;

                // 计算pMin
                if (xmin > meshes[i].vertices[j].Position.x)
                    xmin = meshes[i].vertices[j].Position.x;
                if (ymin > meshes[i].vertices[j].Position.y)
                    ymin = meshes[i].vertices[j].Position.y;
                if (zmin > meshes[i].vertices[j].Position.z)
